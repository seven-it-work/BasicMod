...
public abstract class BaseRelic extends CustomRelic {
    ...
    protected void loadTexture() {
        this.img = TextureLoader.getTextureNull(LotteryMod.resourcePath.relicPath(imageName + ".png"), true);
        if (img != null) {
            outlineImg = TextureLoader.getTextureNull(LotteryMod.resourcePath.relicPath(imageName + "Outline.png"), true);
            if (outlineImg == null)
                outlineImg = img;
        }
        else {
    ...
    public void loadLargeImg() {
        if (notPng(imageName)) {
            if (largeImg == null) {
                this.largeImg = ImageMaster.loadImage(LotteryMod.resourcePath.relicPath("large/" + imageName + ".png"));
            }
        }
        else {
            super.loadLargeImg();
